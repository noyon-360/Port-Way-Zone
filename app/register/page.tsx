...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    try {
      setIsLoading(true);
      const { data } = await authApi.post(ENDPOINTS.AUTH.REGISTER, {
        name,
        email,
        password,
      });
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 overflow-hidden bg-background">
      <Link
        href="/"
        className="absolute top-8 left-8 flex items-center gap-3 z-20 opacity-70 hover:opacity-100 transition-opacity"
      >
        ...
        <div className="glass-card rounded-3xl p-8 sm:p-10 relative overflow-hidden">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-sm text-slate-400">
              Join Portway to streamline your DevOps
            </p>
          </div>
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold tracking-wide text-slate-300 ml-1 uppercase">
                Full Name
              </label>
